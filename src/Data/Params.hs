...
--         filtergo _ = False
    ...
    let tyVarBndrL_Config = filter go tyVarBndrL'
        go (KindedTV _ (AppT (ConT maybe) _)) = nameBase maybe=="Config"
        go _ = False

    let tyVarBndrL_Star = filter go tyVarBndrL'
        go (KindedTV _ StarT ) = True
        go (PlainTV _ ) = True
        go _ = False
